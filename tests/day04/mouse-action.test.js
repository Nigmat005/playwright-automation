...
test.describe("Test Group", async () => {
  ...
  test("Mouse hover test2 with $", async ({ page }) => {
    await page.click("text='Hovers'");
    await page.waitForTimeout(2000);
    const firstAvator=await page.$("(//img[@src='/img/avatar-blank.jpg' and @alt='User Avatar'])");

    await firstAvator.hover();
  ...
  test("hover to multiple test", async ({ page }) => {
    ...
    for (const each_element of await page.$$("//img[@src='/img/avatar-blank.jpg' and @alt='User Avatar']")) {
        await page.waitForTimeout(2000);
        await each_element.hover();
      }
  });
